  ...
  return knex.transaction((transaction) => {
    return transaction.schema.raw('CREATE SCHEMA IF NOT EXISTS ' + 'user_module' + ';').then(() => {
      return transaction.schema
        .hasTable('sub-franchisees')
        .then(function (exists) {
          if (!exists) {
            return transaction.schema.withSchema('user_module').createTable('sub-franchisees', (table) => {
...
export async function down(knex: Knex): Promise<any> {
  return knex.schema.withSchema('user_module').dropTableIfExists('sub-franchisees');
}
